...
freq=0.333; % Frequency 0.3, 0.7, 1.3, 2.7, 3.3, 3.7, etc.
rm1=[0 0.05 0.05 0.05]; % Nonuniform sampling ratio four
...
if(a==3)
    pks = findpeaks(XN2)
[pks,locs] = findpeaks(XN2)
[pks,locs,w,p] = findpeaks(XN2)
   plot(0:0.001:(N/2-1)/N, XN2,'red')
    title(freq)
else
    pks1 = findpeaks(XN1)
[pks1,locs1] = findpeaks(XN1)
[pks1,locs1,w1,p1] = findpeaks(XN1)
    plot(0:0.001:(N/2-1)/N, XN1,'green')
    title(freq)
end
